...
def setup_argparse():
    ...
    parser.add_argument("--epochs", type=int, default=1, help="NUMBER OF ADDITIONAL EPOCHS TO TRAIN.")
    ...
    parser.add_argument("--experiment", type=str, default="", help="Experiment subfolder for outputs and checkpoints")
    parser.add_argument("--exclude-packet", type=str, default=None, help="Packet to exclude from training")
    parser.add_argument("--use-bucketing", action="store_true", help="Experiment B: Use aspect ratio bucketing")
    parser.add_argument("--focal-loss", action="store_true", help="Experiment D: Use Focal CTC Loss")
    parser.add_argument("--less-downsample", action="store_true", help="Experiment C: Reduce CNN spatial downsampling")
    parser.add_argument('--use-multiscale-aug', action='store_true', help='Use random scale and pad augmentation')
    parser.add_argument('--use-res-degradation', action='store_true', help='Use resolution degradation augmentation')
    return parser
...
def save_checkpoint(model, optimizer, state, config, path):
...
def run_validation(model, dataloader, criterion, tokenizer, device, args, debug=False):
    model.eval()
    val_loss = 0
    total_cer = 0
    total_wer = 0
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(dataloader):
            ...
            # Calculate CTC loss
            input_lengths = (actual_widths // (2 if args.less_downsample else 4)).to(torch.long)
            input_lengths = torch.clamp(input_lengths, min=1, max=outputs_log_probs.size(0))
            
            # Using reduction='mean' for validation strictly for evaluation consistency
            val_criterion = nn.CTCLoss(blank=0, zero_infinity=True, reduction='mean')
            loss = val_criterion(outputs_log_probs, targets, input_lengths, target_lengths)
...
def verify_one_batch(model, dataloader, criterion, tokenizer, device, args):
    """
    ...
    raw_loss = criterion(outputs_log_probs, targets, input_lengths, target_lengths)
    if args.focal_loss:
        p_t = torch.exp(-raw_loss)
        gamma = 2.0
        loss = ((1 - p_t) ** gamma * raw_loss).mean()
    else:
        loss = raw_loss.mean() if raw_loss.dim() > 0 else raw_loss
...
def train():
    ...
    # Initialize model
    model = CRNN(tokenizer.num_classes, feature_size=2048, less_downsample=args.less_downsample).to(device)
    
    # Initialize loss
    # We use reduction='none' if focal_loss is enabled to apply per-sequence scaling
    reduction = 'none' if args.focal_loss else 'mean'
    criterion = nn.CTCLoss(blank=0, zero_infinity=True, reduction=reduction)
    ...
    if args.resume:
        ...
        print(f"[VERIFY] Successfully loaded model weights from: {args.resume}")
        out_features = getattr(model, 'head', getattr(model, 'fc', None))
        if out_features is not None and hasattr(out_features, 'linear'):
            out_features = out_features.linear.out_features
        elif hasattr(out_features, 'out_features'):
            out_features = out_features.out_features
        print(f"[VERIFY] Output dimension of model: {out_features} classes (should be vocab size + 1 for CTC blank)")
        ...
        print(f"[VERIFY] Restored model from epoch {start_epoch - 1}. Training will continue starting at epoch {start_epoch}.")
    ...
    try:
        for epoch in range(start_epoch, end_epoch):
            ...
            train_loader = get_dataloader(
                ...
                target_packet=args.packet,
                replay_previous=True if args.packet else False,
                shuffle=True,
                use_bucketing=args.use_bucketing,
                use_multiscale_aug=args.use_multiscale_aug,
                use_res_degradation=args.use_res_degradation
            )
            ...
            if not first_batch_verified:
                if not args.resume:
                    print(f"\nVerifying model forward/backward pass...")
                    verify_one_batch(model, train_loader, criterion, tokenizer, device, args)
            ...
            save_checkpoint(model, optimizer, training_state, config, pre_val_path)
            print(f"[SAVE] Pre-validation checkpoint saved to {pre_val_path}")
            
            # 3. Validate
            val_loss, cer, wer, examples, mc_stats = 0, 0, 0, [], {}
            if val_loader:
                val_loss, cer, wer, examples, mc_stats = run_validation(model, val_loader, criterion, tokenizer, device, args, debug=args.debug)
            ...
            save_checkpoint(model, optimizer, training_state, config, latest_path)
            save_checkpoint(model, optimizer, training_state, config, epoch_path)
            
            if is_best:
                best_path = str(ckpt_dir / "best.pth")
                save_checkpoint(model, optimizer, training_state, config, best_path)
